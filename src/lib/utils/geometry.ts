/**
 * Shared Geometry Generation Logic
 *
...
export function getFaceNormal(faceName: string): [number, number, number] {
    switch (faceName) {
        case "east":
            return [1, 0, 0];
        case "west":
...
export function getFaceUVs(faceData: ElementFace): number[] {
    if (!faceData.uv) {
